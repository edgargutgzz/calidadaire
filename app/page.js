        ...
        {nearestSensor && (
          <div className="mx-4 lg:mx-0 mb-0 pb-4">
            ...
            <div className="fixed bottom-0 left-0 right-0 mb-16 flex justify-between mt-4 py-2 px-4 shadow-lg">
              {/* Vulnerable */}
              <div 
                className={`flex text-sm items-center justify-center rounded-2xl w-1/2 mr-2 p-2 cursor-pointer text-center ${selectedProfile === 'vulnerable' ? 'bg-black text-white' : 'bg-white shadow-lg border-transparent'}`}
                onClick={() => setSelectedProfile('vulnerable')}>
                Población Vulnerable
              </div>
              {/* General */}
              <div 
                className={`flex text-sm items-center justify-center rounded-2xl w-1/2 ml-2 p-2 cursor-pointer text-center ${selectedProfile === 'general' ? 'bg-black text-white' : 'bg-white shadow-lg border-transparent'}`}
