...
function Navbar({ currentPage }) {
  return (
    <nav style={{ boxShadow: '0 -4px 5px -2px rgba(0, 0, 0, 0.1)' }} className="bg-white z-10 fixed w-full bottom-0 flex-col shadow-lg lg:z-auto lg:w-3/12 lg:min-h-screen lg:static lg:flex lg:flex-col lg:py-10">
      <div className="container mx-auto px-2 pt-3 pb-2 flex justify-around items-center h-full lg:flex-col lg:justify-start lg:items-start lg:pl-4 lg:pr-5">
        ...
        <div className="text-black lg:hover:bg-gray-100 rounded-lg lg:w-full lg:mb-4 lg:pr-6">
          <Link href="/acerca">
            <div className="flex flex-col items-center text-xs lg:text-base lg:flex-row lg:py-3 lg:p-4">
              <img src={currentPage === 'acerca' ? "/info_on.png" : "/info_off.png"} alt="Inicio" width={22} height={22} className="lg:w-6 lg:h-6" />
              <span className={`mt-0 lg:mt-0 lg:ml-4 pt-1 ${currentPage === 'acerca' ? "font-bold" : ""}`}>Acerca</span>
            </div>
          </Link>
        </div>
